    ...
    try:
        ...
        for current_ip in ip_addresses_to_try:
            ...
            for attempt in connection_attempts:
                try:
                    logger.info(
                        f"Trying {ip_label} connection to {device_name} ({current_ip}):4370 (Password={attempt['password']}, UDP={attempt['force_udp']})"
                    )
                    zk = ZK(current_ip, port=port, timeout=timeout, **attempt)
                    conn = zk.connect()
                    logger.info(
                        f"Successfully connected to {device_name} ({current_ip}):{port} via {ip_label} IP"
                    )
                    return conn
        ...
        logger.error(
            f"Failed to connect to device at {device_name} ({ip_address}):{port}: {str(e)}"
        )
        raise


def fetch_attendance(
    device: Device, since: datetime = None, timeout: int = None
) -> Tuple[int, int]:
    """
    ...
    try:
        ...
        for attendance in attendances:
            ...
            device_tz_name = (
                getattr(device, "timezone", None)
                or getattr(settings, "DEVICE_TIME_ZONE", None)
                or "Africa/Nairobi"
            )
            try:
        ...
        logger.info(
            f"Stored {new_records} new records from {device.name} (total fetched: {total_fetched})"
        )
...
def poll_all_devices(since: datetime = None) -> dict:
    ...
    results = {"total_devices": devices.count(), "successful": 0, "failed": 0, "devices": []}
    ...
    for device in devices:
        try:
            ...
            results["failed"] += 1
            results["devices"].append({"device": device.name, "status": "failed", "error": str(e)})
...
def test_device_connection(ip_address, port=4370):
    ...
    for attempt in connection_attempts:
        try:
            logger.info(
                f"Testing connection to {ip_address}:{port} (Password={attempt['password']}, UDP={attempt['force_udp']})"
            )
    ...
    logger.error(
        f"Connection failed after all attempts. Check device password, IP, and network connectivity."
    )
