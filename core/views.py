...
class DeviceViewSet(viewsets.ModelViewSet):
    ...
    def info(self, request, pk=None):
        ...
        if info:
            ...
            return Response(
                {"error": "Failed to connect to device"}, status=status.HTTP_503_SERVICE_UNAVAILABLE
            )
...
def attendance_report(request):
    ...
    normal_count = len(
        [
            ...
            and not d["is_late_arrival"]
            and not d["is_early_departure"]
            and not d["is_incomplete"]
        ]
    )
...
def delete_attendance(request):
    ...
    for ts in to_delete:
        qs = RawAttendance.objects.filter(
            device=processed.device, user_id=processed.user_id, timestamp=ts
        )
    ...
    remaining = RawAttendance.objects.filter(
        device=processed.device, user_id=processed.user_id, timestamp__date=shift_date
    ).exists()
    ...
    if not remaining:
        ...
        try:
            ...
            return JsonResponse(
                {"deleted": deleted_count, "processed_delete_error": str(e)}, status=500
            )
    else:
...
def attendance_print(request):
    ...
    full_name_subquery = DeviceUser.objects.filter(
        user_id=OuterRef("user_id"), device=OuterRef("device")
    ).values("full_name")[:1]
    ...
    if search_query:
        ...
        attendances = attendances.filter(
            Q(user_id__icontains=search_query)
            | Q(
                user_id__in=DeviceUser.objects.filter(
                    Q(name__icontains=search_query) | Q(full_name__icontains=search_query)
...
def sync_day(request):
    ...
    outliers_before = OutlierPunch.objects.filter(punch_datetime__date=sync_date)
    if device_id:
    ...
    for device in devices:
        device_result = {"name": device.name, "raw_fetched": 0, "processed": 0, "error": None}

        try:
            # Check if we already have data for this date from this device
            existing_count = RawAttendance.objects.filter(
                device=device, timestamp__date=sync_date
            ).count()
            ...
            if existing_count > 0:
                ...
                try:
                    ...
                    total_fetched, new_records = fetch_attendance(
                        device, since=since_dt, timeout=fetch_timeout
                    )
    ...
    outliers_after = OutlierPunch.objects.filter(punch_datetime__date=sync_date)
    if device_id:
...
def dashboard(request):
    ...
    raw_attendances = (
        RawAttendance.objects.filter(timestamp__date=selected_date)
        .annotate(
            user_name_display=Coalesce(
                Subquery(full_name_subquery, output_field=CharField()),
                Subquery(name_subquery, output_field=CharField()),
    ...
    if employee_filter:
        raw_attendances = raw_attendances.filter(
            ...
            | Q(
                user_id__in=DeviceUser.objects.filter(
                    Q(name__icontains=employee_filter) | Q(full_name__icontains=employee_filter)
                ).values_list("user_id", flat=True)
            )
        )
...
def device_edit(request, device_id):
    ...
    context = {"device": device, "action": "Edit"}
...
def device_users(request, device_id):
    ...
    missing_from_device = DeviceUser.objects.filter(
        device=device, user_id__in=missing_from_device_ids
    )

    # Get last attendance for each user on this device
    last_attendance = {}
    attendance_data = (
        RawAttendance.objects.filter(device=device)
        .values("user_id")
        .annotate(last_punch=Max("timestamp"))
    )
...
def device_user_edit(request, device_id, user_id):
    ...
    context = {"device": device, "device_user": device_user}
...
def device_user_delete(request, device_id):
    ...
    if request.method == "POST":
        try:
            ...
            return JsonResponse({"success": True, "deleted": success_count, "failed": failed_count})
        except Exception as e:
...
def device_user_delete_from_db(request, device_id):
    ...
    if request.method == "POST":
        try:
            ...
            deleted_count = DeviceUser.objects.filter(device=device, user_id__in=user_ids).delete()[
                0
            ]
...
def reprocess_attendance(request):
    ...
    try:
        ...
        while current_date <= end_date:
            if device:
                user_ids = (
                    ProcessedAttendance.objects.filter(date=current_date, device=device)
                    .values_list("user_id", flat=True)
                    .distinct()
                )
            else:
                user_ids = (
                    ProcessedAttendance.objects.filter(date=current_date)
                    .values_list("user_id", flat=True)
                    .distinct()
                )
...
def outlier_session_punches(request):
    ...
    for punch in punches:
        ...
        result.append(
            {"punch_time": local_dt.strftime("%Y-%m-%d %H:%M:%S"), "duration_since_prev": duration}
        )
        prev_local = local_dt
...
def user_detail(request, user_id):
    ...
    # User profile
    device_user = DeviceUser.objects.select_related("device").filter(user_id=user_id).first()
    ...
    attendance_qs = (
        ProcessedAttendance.objects.filter(
            user_id=user_id, shift_date__gte=date_from_obj, shift_date__lte=date_to_obj
        )
        .select_related("device")
        .order_by("-shift_date")
    )
...
def outlier_session_detail(request, outlier_id):
    ...
    processed = ProcessedAttendance.objects.filter(
        user_id=rep.user_id, device_id=rep.device_id, shift_date=rep.associated_shift_date
    ).first()

    # Categorise outlier punches by whether they fall before or after the shift window
    before_shift = [
        p for p in all_punches if "early" in p.reason.lower() or "before" in p.reason.lower()
    ]
    after_shift = [
        p for p in all_punches if "late" in p.reason.lower() or "after" in p.reason.lower()
    ]
...
def bulk_sync_attendance(request):
    ...
    try:
        ...
        updated_count = ProcessedAttendance.objects.filter(id__in=attendance_ids).update(
            synced_to_crm=synced, last_sync_attempt=timezone.now() if synced else None
        )
...
def get_user_timezone(request):
    ...
    return JsonResponse({"timezone": tz_name, "current_time": current_time})
