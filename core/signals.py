...
def notify_new_outlier(sender, instance, created, **kwargs):
    ...
    if not email_enabled:
        logger.debug(
            f"Email notifications disabled. Skipping notification for outlier {instance.id}"
        )
        return
...
"""
    ...
    try:
        ...
        logger.info(
            f"Sent outlier notification email for user {instance.user_id} to {len(recipients)} recipient(s)"
        )
