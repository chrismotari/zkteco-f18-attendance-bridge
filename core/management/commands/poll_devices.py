...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--since-hours", type=int, help="Only fetch records from the last N hours"
        )
        parser.add_argument(
            "--since-days", type=int, help="Only fetch records from the last N days"
        )
        ...
        try:
            ...
            for device_result in results["devices"]:
                if device_result["status"] == "success":
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"  ✗ {device_result['device']}: {device_result['error']}")
                    )
