    ...
    def handle(self, *args, **options):
        ...
        for device in devices:
            ...
            try:
                ...
                conn.disconnect()
                self.stdout.write(
                    self.style.SUCCESS(f"Successfully synced users from {device.name}")
                )

            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f"Error syncing users from {device.name}: {str(e)}")
                )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"\nSync complete: {total_synced} users synced ({total_created} created, {total_updated} updated)"
            )
        )
