...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--ip", type=str, help="IP address to test (tests without saving to database)"
        )
        parser.add_argument("--port", type=int, default=4370, help="Port number (default: 4370)")
        parser.add_argument("--device-id", type=int, help="Test specific device from database")
        parser.add_argument("--all", action="store_true", help="Test all devices in database")
    ...
    def handle(self, *args, **options):
        if options["ip"]:
            ...
            try:
                ...
                self.stdout.write(
                    f"Testing device: {device.name} ({device.ip_address}:{device.port})"
                )
