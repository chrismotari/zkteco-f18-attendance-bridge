...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--limit", type=int, help="Maximum number of records to sync")
        parser.add_argument(
            "--start-date", type=str, help="Start date (YYYY-MM-DD) for date range sync"
        )
        parser.add_argument(
            "--end-date", type=str, help="End date (YYYY-MM-DD) for date range sync"
        )
        parser.add_argument("--user-id", type=str, help="Sync only for specific user ID")
        parser.add_argument(
            "--retry-failed", action="store_true", help="Retry previously failed syncs"
        )
        parser.add_argument(
            "--force", action="store_true", help="Force resync even already synced records"
        )
        ...
        try:
            if options["retry_failed"]:
                ...
                results = sync_by_date_range(
                    start_date, end_date=end_date, force_resync=options["force"]
                )
