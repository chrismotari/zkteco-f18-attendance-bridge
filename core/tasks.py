...
def poll_devices_task():
    ...
    try:
        ...
        logger.info(
            f"Device polling completed: {results['successful']} devices polled successfully, "
            f"{results['failed']} failed"
        )

        return {"status": "success", "results": results, "timestamp": timezone.now().isoformat()}
...
def process_attendance_task():
    ...
    try:
        ...
        logger.info(
            f"Attendance processing completed: {results['processed']} records processed, "
...
def sync_to_crm_task(limit=None):
    ...
    try:
        ...
        logger.info(
            f"CRM sync completed: {results['successful']} records synced, {results['failed']} failed"
        )
...
def retry_failed_syncs_task():
    ...
    try:
        ...
        logger.info(
            f"Retry failed syncs completed: {results['successful']} records synced, {results['failed']} still failed"
        )
...
def daily_cleanup_task():
    ...
    results = {"old_raw_deleted": 0, "duplicates_removed": 0}
...
def process_date_range_task(start_date_str: str, end_date_str: str = None):
    ...
    logger.info(
        f"Processing attendance for date range: {start_date_str} to {end_date_str or 'today'}"
    )
