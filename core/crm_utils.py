...
def send_to_crm(attendance: ProcessedAttendance, retry_on_failure: bool = True) -> Tuple[bool, str]:
    ...
    try:
        ...
        response = session.post(
            settings.CRM_API_URL, json=data, headers=headers, timeout=settings.CRM_REQUEST_TIMEOUT
        )

        # Check response status
        if response.status_code in [200, 201]:
            logger.info(
                f"Successfully synced attendance: User {attendance.user_id}, Date {attendance.date}"
            )
...
def sync_batch(attendances: List[ProcessedAttendance], delay_between: float = 0.1) -> dict:
    ...
    """
    results = {"total": len(attendances), "successful": 0, "failed": 0, "errors": []}
    ...
    for attendance in attendances:
        ...
        if success:
            ...
            results["errors"].append(
                {"user_id": attendance.user_id, "date": str(attendance.date), "error": message}
            )
    ...
    logger.info(
        f"Batch sync complete: {results['successful']} successful, {results['failed']} failed"
    )
    return results
...
def sync_unsynced_attendance(limit: int = None) -> dict:
    ...
    unsynced = ProcessedAttendance.objects.filter(synced_to_crm=False).order_by("date", "user_id")[
        :limit
    ]
...
def sync_by_date_range(start_date: date, end_date: date = None, force_resync: bool = False) -> dict:
    ...
    logger.info(
        f"Syncing attendance from {start_date} to {end_date} ({'force resync' if force_resync else 'unsynced only'})"
    )
...
def sync_by_user(
    user_id: str, start_date: date = None, end_date: date = None, force_resync: bool = False
) -> dict:
    """
    ...
    logger.info(
        f"Syncing attendance for user {user_id} ({'force resync' if force_resync else 'unsynced only'})"
    )
...
def retry_failed_syncs(max_attempts: int = 3) -> dict:
    ...
    failed = ProcessedAttendance.objects.filter(
        synced_to_crm=False, sync_attempts__gt=0, sync_attempts__lt=max_attempts
...
def test_crm_connection() -> Tuple[bool, str]:
    ...
    try:
        ...
        response = requests.get(
            settings.CRM_API_URL, headers=headers, timeout=settings.CRM_REQUEST_TIMEOUT
        )
        ...
        if response.status_code in [
            ...
            201,
            405,
        ]:  # 405 = Method not allowed (but connection works)
