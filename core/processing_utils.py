...
def classify_attendance(
    ...
    if clock_in:
        ...
        if clock_in_time > work_start:
            ...
            minutes_late = (
                datetime.combine(date.today(), clock_in_time)
                - datetime.combine(date.today(), work_start)
            ).total_seconds() / 60
            reasons.append(
                f"Late arrival: clocked in at {clock_in_time.strftime('%H:%M')} ({int(minutes_late)} min late)"
            )
        ...
        if overnight_shift and work_end.hour < 12:
            ...
            if clock_out_time.hour < 12 and clock_out_time < work_end:
                ...
                minutes_early = (
                    ...
                    - datetime.combine(date.today(), clock_out_time)
                ).total_seconds() / 60
                reasons.append(
                    f"Left early: clocked out at {clock_out_time.strftime('%H:%M')} ({int(minutes_early)} min early)"
                )
        else:
...
def is_punch_in_shift_window(
    punch_datetime: datetime,
    shift_date: date,
    work_start: time,
    work_end: time,
    overnight_shift: bool,
    device_timezone: str,
    buffer_hours: int = 2,
) -> bool:
    """
...
def determine_outlier_reason(
    ...
    if overnight_shift:
        ...
        earliest_acceptable = (
            datetime.combine(date.today(), work_start) - timedelta(hours=buffer_hours)
        ).time()
        next_day_latest = (
            datetime.combine(date.today(), work_end) + timedelta(hours=buffer_hours)
        ).time()

        # Check if it's way before shift start
        if punch_time < earliest_acceptable and punch_time.hour >= 12:
            hours_early = (
                datetime.combine(date.today(), earliest_acceptable)
                - datetime.combine(date.today(), punch_time)
            ).total_seconds() / 3600
        ...
        # Check if it's after morning window ends
            ...
            hours_late = (
                datetime.combine(date.today(), punch_time)
                - datetime.combine(date.today(), next_day_latest)
            ).total_seconds() / 3600
        ...
        if punch_time < earliest_acceptable:
            ...
            hours_late = (
                datetime.combine(date.today(), punch_time)
                - datetime.combine(date.today(), latest_acceptable)
            ).total_seconds() / 3600
            return f"Punch at {punch_time.strftime('%H:%M')} is {hours_late:.1f} hours after shift ends"


def process_attendance_for_date(
    user_id: str, attendance_date: date, device: Device = None
    ...
    if overnight_shift:
        ...
        query_start_local = datetime.combine(attendance_date, work_start) - timedelta(
            hours=outlier_early_in + 2
        )
        ...
        query_start_local = datetime.combine(attendance_date, work_start) - timedelta(
            hours=buffer_hours + 2
        )
        query_start = tz.localize(query_start_local)

        query_end_local = datetime.combine(attendance_date, work_end) + timedelta(
            hours=buffer_hours + 2
        )
        query_end = tz.localize(query_end_local)

    # Query for all punches in the window
    query = Q(user_id=user_id, timestamp__gte=query_start, timestamp__lte=query_end)
    ...
    for punch in out_of_window_punches:
        reason = determine_outlier_reason(
            punch.timestamp, attendance_date, work_start, work_end, overnight_shift, device_tz
        )
    ...
    if not in_window_punches:
        logger.warning(
            f"No in-window punches for user {user_id} on {attendance_date}, "
    ...
    logger.info(
        f"{action} processed attendance for user {user_id} on {attendance_date}: "
...
def process_all_unprocessed_attendance() -> dict:
    ...
    results = {"total_records": 0, "processed": 0, "failed": 0, "outliers": 0}
    ...
    if overnight_shift:
        ...
        unique_combinations = (
            RawAttendance.objects.values("user_id", "device_id", "timestamp__date")
            .distinct()
            .order_by("user_id", "device_id", "timestamp__date")
        )

        logger.info(
            f"Found {unique_combinations.count()} unique user/device/date combinations to process"
        )
        ...
        for combo in unique_combinations:
            ...
            try:
                ...
                evening_punches = RawAttendance.objects.filter(
                    user_id=user_id, device_id=device_id, timestamp__date=punch_date
                ...
                results["failed"] += 1
                logger.error(
                    f"Error processing attendance for user {user_id} on {punch_date}: {str(e)}"
                )
    else:
        ...
        for combo in unique_combinations:
            ...
            try:
                ...
                results["failed"] += 1
                logger.error(
                    f"Error processing attendance for user {user_id} on {attendance_date}: {str(e)}"
                )
...
def process_attendance_for_date_range(
    start_date: date, end_date: date = None, device: Device = None
) -> dict:
    """
    ...
    raw_records = (
        RawAttendance.objects.filter(query)
        .values("user_id", "timestamp__date", "device")
        .distinct()
    )
    ...
    logger.info(
        f"Date range processing complete: {results['processed']} processed, "
        f"{results['outliers']} outliers, {results['failed']} failed"
    )
...
def process_all_attendance_for_date(target_date: date, device_id: int = None) -> int:
    ...
    unique_combinations = (
        RawAttendance.objects.filter(**query_filter).values("user_id", "device_id").distinct()
    )
    ...
    for combo in unique_combinations:
        ...
        try:
            ...
            logger.error(
                f"Error processing attendance for user {user_id} on {target_date}: {str(e)}"
            )
...
def deduplicate_records() -> dict:
    ...
    duplicates = (
        ProcessedAttendance.objects.values("user_id", "date", "device")
        .annotate(count=Count("id"))
        .filter(count__gt=1)
    )

    results = {"duplicate_groups": duplicates.count(), "records_removed": 0}
    ...
    for dup in duplicates:
        ...
        records = ProcessedAttendance.objects.filter(
            user_id=dup["user_id"], date=dup["date"], device_id=dup["device"]
        ).order_by("-updated_at")
...
def get_failed_sync_attendance(max_attempts: int = 3) -> List[ProcessedAttendance]:
    ...
    queryset = ProcessedAttendance.objects.filter(
        synced_to_crm=False, sync_attempts__gte=max_attempts
